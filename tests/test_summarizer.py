...
class TestYoutubeTextSummarizer:
    ...
    def test_token_limit_exceeded(self, mock_env, capsys):
        ...
        assert "token limit exceeded" in capsys.readouterr().out.lower()

    def test_api_failure(self, mock_env, capsys):
    ...
    def test_resource_exhausted(self, mock_env, capsys):
        ...
        assert summary is None
        assert "Error: API quota exceeded (Resource Exhausted)" in capsys.readouterr().out